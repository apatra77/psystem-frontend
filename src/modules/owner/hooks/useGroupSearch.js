...
const AUTocomplete_PAGE_SIZE = GROUPS_PAGE_SIZE
...
export function useGroupSearch(query = '') {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const payload = await fetchGroupsSearch({
          query: trimmed,
          page: 0,
          size: AUTocomplete_PAGE_SIZE,
        })
        const result = parseGroupsPage(payload)
        ...
        setIsLast(true)
        setPage(0)
        setError(err instanceof Error ? err.message : 'Failed to search MFR/MKT names')
      } finally {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      setSuggestions((prev) => {
        const merged = mergeGroupItems(prev, result.items)
