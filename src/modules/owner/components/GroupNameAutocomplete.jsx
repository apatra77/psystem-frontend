...
function AddGroupConfirmModal({ name, onClose, onConfirm, saving, error }) {
  return (
    <PortalModal onClose={onClose} width={420} scrollable={false} closeOnBackdrop={!saving} zIndex={320}>
      <div className="p-6">
        <div className="text-[17px] font-extrabold text-white mb-2">Add MFR/MKT name?</div>
        ...
        <p className="text-[12px]" style={{ color: colors.textDim }}>
          This manufacturer/marketer is not in the catalog yet.
        </p>
...
export default function GroupNameAutocomplete({
  value = '',
  onChange,
  disabled = false,
  ...
  const handleConfirmAdd = async () => {
    ...
    try {
      const created = await createGroup({ groupName: trimmedInput })
      toast.success(`MFR/MKT name "${created.name}" added`)
      commitSelection(created)
      setConfirmOpen(false)
    } catch (err) {
      setCreateError(err instanceof Error ? err.message : 'Could not add MFR/MKT name')
    } finally {
      setCreating(false)
    }
  }
        ...
        {showDropdown && (
          ...
          >
            {loading || isSearchPending ? (
              ...
              <div className="px-3 py-2.5 text-[12px] font-bold text-red-400">{searchError}</div>
            ) : showEmptyAddState ? (
              <>
